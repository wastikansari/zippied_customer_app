...
//                         itemCount: paymentModeProvider.availablePaymentModes.length,
//                         itemBuilder: (context, index) {
//                           final mode = paymentModeProvider.availablePaymentModes[index];
...
//         ChangeNotifierProvider.value(value: Provider.of<OrderProvider>(context)),
//         ChangeNotifierProvider.value(value: Provider.of<PaymentModeProvider>(context)),
//       ],
//       child: Consumer4<WalletProvider, AddressProvider, OrderProvider, PaymentModeProvider>(
//         builder: (context, walletProvider, addressProvider, orderProvider, paymentModeProvider, child) {
//           if (paymentModeProvider.isLoading) {
//             return const Center(child: CircularProgressIndicator());
//           }
//           if (paymentModeProvider.errorMessage != null) {
//             return Center(child: CustomText(text: paymentModeProvider.errorMessage!, color: Colors.red));
//           }

//           final walletBalance = walletProvider.walletBalance?.data?.wallet?.totalBalance?.toInt() ?? 0;
//           final defaultAddress = addressProvider.addresses.firstWhere(
//             (address) => address.isPrimary == true,
//             orElse: () => addressProvider.addresses.isNotEmpty
//                 ? addressProvider.addresses.first
...
//           );
//           final charges = {
//             'service_charge': widget.bookingDetails['service_charges'] as int? ?? 169,
//             'slot_charge': widget.bookingDetails['slot_charges'] as int? ?? 0,
//             'tips': _selectedTip,
//             'handling_charge': handlingCharge,
//           };
//           final totalPayable = charges.values.reduce((a, b) => a + b) + 12; // Adding GST as per screenshot
//           final payableAfterWallet = _useWallet
//               ? (totalPayable > walletBalance ? totalPayable - walletBalance : 0)
//               : totalPayable;
...
//             appBar: const PreferredSize(
//               preferredSize: Size.fromHeight(60),
//               child: CustomAppBar(title: "Review Booking", isBack: true),
//             ),
//             body: walletProvider.isLoading || addressProvider.isLoading
//                 ? const Center(child: CircularProgressIndicator())
...
//               ),
//               child: ContinueButton(
//                 text: payableAfterWallet > 0 && (_selectedPaymentMode != 'Cash' || _selectedPaymentMode == null)
//                     ? 'Proceed to pay ₹$payableAfterWallet'
//                     : 'Confirm Booking',
//                 isValid: defaultAddress.addressId != null && !_isProcessing && _selectedPaymentMode != null,
//                 isLoading: walletProvider.isLoading || addressProvider.isLoading || _isProcessing,
//                 onTap: () {
//                   if (_isNavigating || _isProcessing) return;
//                   if (defaultAddress.addressId == null) {
//                     _showToast('Please select an address');
...
//                       context,
//                       MaterialPageRoute(
//                         builder: (context) => AddressScreen(selectedAddressId: defaultAddress.addressId),
//                       ),
//                     ).then((_) => _isNavigating = false);
//                     return;
//                   }
//                   _processPayment(walletBalance, totalPayable, defaultAddress.addressId!, _selectedPaymentMode!, paymentModeProvider, context);
//                 },
//               ),
//             ),
//           );
...
//           visualDensity: const VisualDensity(horizontal: 0, vertical: 0),
//           leading: Image.asset(AppAssets.offerIcon, height: 30, color: Colors.green),
//           title: CustomText(text: "Apply coupons or offers", size: 14, fontweights: FontWeight.w500),
//           trailing: const Icon(Icons.arrow_forward_ios_sharp),
//           onTap: () {
...
//           title: CustomText(text: "Redeem using wallet", size: 15, fontweights: FontWeight.w500),
//           subtitle: Row(
//             mainAxisAlignment: MainAxisAlignment.start,
//             crossAxisAlignment: CrossAxisAlignment.start,
//             children: [
...
//           crossAxisAlignment: CrossAxisAlignment.start,
//           children: [
//             CustomText(text: "Service Refund & cancellation policy", size: 16, fontweights: FontWeight.w500),
//             const Height(8),
//             CustomText(
//               text: "Free cancellations/refund if done more than 30 mins before the service. A fee will be charged otherwise",
//               size: 12,
//               color: Colors.grey,
//             ),
//             const Height(8),
//             GestureDetector(
//               onTap: () {
//                 _showToast("Learn more tapped");
//               },
//               child: CustomText(text: "Learn more", size: 12, color: AppColor.appbarColor),
//             ),
//           ],
//         ),
//       ),
...
//   Widget _buildPaymentDetails(Map<String, int> charges, int totalPayable) {
//     final gst = (charges['service_charge']! * 0.18).round(); // 18% GST as per screenshot
...
//           crossAxisAlignment: CrossAxisAlignment.start,
//           children: [
//             CustomText(text: "Payment details", size: 16, fontweights: FontWeight.w500),
//             const Height(10),
//             _buildChargeRow("Service Charge", charges['service_charge']!),
//             const Height(10),
//             _buildChargeRow("GST", gst),
//             const Height(10),
//             const dotPointWidget(),
//             const Height(10),
//             _buildChargeRow("Total Payable", totalPayable + gst, isTotal: true),
//           ],
//         ),
//       ),
//     );
...
//                 Row(
//                   children: [
//                     Image.asset(AppAssets.wallet, height: 24, color: Colors.grey),
//                     const SizedBox(width: 8),
//                     CustomText(text: "Payment method", size: 16, fontweights: FontWeight.w500),
//                     const SizedBox(width: 8),
//                     CustomText(text: _selectedPaymentMode ?? "Select", size: 14, color: Colors.grey),
//                   ],
//                 ),
//                 const Icon(Icons.arrow_forward_ios_sharp, size: 16, color: Colors.grey),
//               ],
//             ),
//           ),
//         ),
...
//   Widget _buildChargeRow(String title, int amount, {bool isTotal = false, Color? color}) {
//     return Row(
//       mainAxisAlignment: MainAxisAlignment.spaceBetween,
//       children: [
//         CustomText(
...
//   void _processPayment(int walletBalance, int totalPayable, String addressId, String selectedPaymentMode,
//       PaymentModeProvider paymentModeProvider, BuildContext context) async {
//     if (_isProcessing) return;
//     setState(() {
//       _isProcessing = true;
//     });
...
//       return;
//     }
//     if (widget.bookingDetails['service_id'] == null || widget.bookingDetails['garment_qty'] == null) {
//       _showToast('Invalid booking details');
//       setState(() {
//         _isProcessing = false;
//       });
...
//     final bookingDetails = Map<String, dynamic>.from(widget.bookingDetails);
//     bookingDetails['address_id'] = addressId;
//     bookingDetails['handling_charges'] = handlingCharge;
//     bookingDetails['tip_amount'] = _selectedTip;
//     bookingDetails['slot_charges'] = widget.bookingDetails['slot_charges'] ?? 0;
//     bookingDetails['total_billing'] = totalPayable;
//     bookingDetails['payment_mode'] = selectedPaymentMode;
//     bookingDetails['payment_status'] = selectedPaymentMode == 'Cash' ? 'Unpaid' : 'Unpaid';
//     bookingDetails['transaction_id'] = 'TXN_${DateTime.now().millisecondsSinceEpoch}';

//     final orderProvider = Provider.of<OrderProvider>(context, listen: false);
//     try {
//       if (selectedPaymentMode == 'Cash') {
//         final orderResponse = await orderProvider.createBooking(bookingDetails);
//        // _showToast('Booking successful: ${orderResponse.data?.order?.orderDisplayNo}');
//         Navigator.push(context, MaterialPageRoute(builder: (context) => const BottomNavigation()));
//       } else if (selectedPaymentMode == 'Wallet') {
//         if (_useWallet && walletBalance >= totalPayable) {
//           await Provider.of<WalletProvider>(context, listen: false).debit(
//             amount: totalPayable,
//             transactionId: bookingDetails['transaction_id'],
//             gatewayResponse: 'success',
//             reason: 'booking ${bookingDetails['order_display_no'] ?? 'ORD'}',
//             message: 'Payment of ₹$totalPayable for booking',
//           );
...
//           onSuccess: () {
//             bookingDetails['payment_status'] = 'Paid';
//             orderProvider.createBooking(bookingDetails).then((_) {
